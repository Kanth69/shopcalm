...
@section('subheader', 'Product category classification')

@section('content')
<div class="card bg-white border-0 shadow-sm rounded-4 mb-4 p-3">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
        <form method="GET" action="{{ route('product-manager.categories.index') }}" class="d-flex gap-2 flex-grow-1" style="max-width: 400px;">
            <div class="input-group">
                <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
                <input type="text" name="search" class="form-control border-start-0 ps-0" placeholder="Search category..." value="{{ request('search') }}" style="font-size: 0.85rem;">
            </div>
            <button type="submit" class="btn btn-primary px-3" style="font-size: 0.85rem;">Search</button>
            @if(request()->filled('search'))
                <a href="{{ route('product-manager.categories.index') }}" class="btn btn-light border"><i class="bi bi-x-lg"></i></a>
            @endif
        </form>

        <a href="{{ route('product-manager.categories.create') }}" class="btn btn-pm-primary rounded-pill px-3" style="font-size: 0.85rem;">
                    ...
                    <tr>
                        <td class="ps-4">
                            <div class="d-flex align-items-center gap-3">
                                @if($cat->image)
                                    <img src="{{ asset('storage/' . $cat->image) }}" class="rounded-3 border" style="width: 38px; height: 38px; object-fit: cover;">
                                @else
                                ...
                                @endif
                                <div class="fw-bold text-dark" style="font-size: 0.88rem;">{{ $cat->name }}</div>
                            </div>
                        </td>
                        ...
                        <td class="pe-4 text-end">
                            <a href="{{ route('product-manager.categories.edit', $cat) }}" class="btn btn-sm btn-light border rounded-pill px-3" style="font-size: 0.78rem;">
                                Edit
                            </a>
                        </td>
                    </tr>
