...
@section('title', $collection->name . ' - Shopcalm')
    ...
    <div class="row">
        ...
        <div class="col-12">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden position-relative" style="background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%);">
                @if($collection->image)
                    <img src="{{ asset('storage/' . $collection->image) }}" class="position-absolute w-100 h-100 object-fit-cover opacity-25" style="mix-blend-mode: overlay;">
                @endif
                <div class="card-body p-5 position-relative z-1 text-white text-center">
                    <h1 class="display-4 fw-bolder mb-2 text-white">{{ $collection->name }}</h1>
                    @if($collection->description)
                        <p class="lead mb-0 opacity-75 mx-auto" style="max-width: 600px;">{{ $collection->description }}</p>
                    @endif
                </div>
            </div>

            <div class="d-flex justify-content-between align-items-center mb-4">
                <p class="text-muted mb-0 fw-medium">{{ number_format($products->total()) }} items matching this collection</p>
            ...
            <div class="row content-loaded d-none g-4">
                ...
                @empty
                    <div class="col-12 text-center py-5 bg-white rounded-4 shadow-sm">
                        ...
                        <p class="text-muted">There are currently no active products in this collection.</p>
