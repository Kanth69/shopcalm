...
<div class="rounded-4 mb-4 overflow-hidden position-relative"
    ...
    <div class="position-absolute inset-0 w-100 h-100"
         style="background-image: radial-gradient(circle, rgba(255,255,255,0.04) 1px, transparent 1px); background-size:24px 24px; top:0; left:0; right:0; bottom:0;"></div>
    <div class="position-relative p-4 d-flex align-items-center gap-4 flex-wrap">
        ...
        <div class="flex-grow-1">
            ...
            <p class="text-white-50 small mb-0">Track your shipments, view receipts & manage purchases</p>
        </div>
        {{-- Quick stats passed from controller --}}
        <div class="d-flex gap-3 flex-wrap">
            <div class="text-center px-3 py-2 rounded-3" style="background:rgba(255,255,255,0.07); border:1px solid rgba(255,255,255,0.1);">
            ...
            <div class="text-center px-3 py-2 rounded-3" style="background:rgba(16,185,129,0.15); border:1px solid rgba(16,185,129,0.3);">
            ...
            <div class="text-center px-3 py-2 rounded-3" style="background:rgba(251,191,36,0.15); border:1px solid rgba(251,191,36,0.3);">
...
<div class="card border-0 shadow-sm rounded-4 mb-4" style="border:1px solid #e2e8f0 !important;">
    <div class="card-body p-3">
        <form method="GET" action="{{ route('account.orders.index') }}" class="d-flex gap-2 align-items-center">
            ...
            <button type="submit" class="btn btn-primary rounded-pill px-4 fw-semibold" style="font-size:0.85rem; white-space:nowrap;">
                Search
            </button>
            @if(request('search'))
...
<p class="text-muted small mb-3 px-1">
    Showing <strong class="text-dark">{{ $orders->firstItem() }}–{{ $orders->lastItem() }}</strong> of <strong class="text-dark">{{ $orders->total() }}</strong> order(s)
</p>
        ...
        <div class="flex-grow-1 p-4">
            ...
            <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap">
                ...
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    ...
                    <div class="d-flex align-items-center position-relative" style="height:52px;">
                        @forelse($thumbs as $ti => $img)
                        <div class="rounded-3 border border-2 border-white overflow-hidden flex-shrink-0"
                             style="width:50px; height:50px; margin-left:{{ $ti > 0 ? '-14px' : '0' }}; z-index:{{ 10 - $ti }}; position:relative; box-shadow:0 1px 4px rgba(0,0,0,0.12);">
            ...
            <div class="mt-4 pt-3" style="border-top:1px dashed #e2e8f0;">
                <div class="position-relative d-flex align-items-center justify-content-between" style="padding:0 2px;">
                    ...
                    {{-- Filled track --}}
                    @php $fillPct = $stepIdx > 0 ? ($stepIdx / (count($steps) - 1)) * 100 : 0; @endphp
                    <div class="position-absolute" style="top:13px; left:14px; width:calc({{ round($fillPct) }}% - 0px); max-width:calc(100% - 28px); height:3px; background:{{ $st['stripe'] }}; border-radius:99px; z-index:1; transition:width 0.5s ease;"></div>
